    ...
    private void setUp(Machine M, String settings) throws EnigmaException {
        try {
            ...
            if (notches.length() != M.numRotors() - 1) {
                throw new EnigmaException("Wrong amount of notches");
            }
            for (char k : notches.toCharArray()) {
                if (!_alphabet.contains(k)) {
                    throw new EnigmaException("Bad char in wheel settings");
                }
            }
            M.setRotors(notches);
            ...
            if (s.split(" ").length != M.numRotors() + 2) {
                throw new EnigmaException("Bad number of arguments");
            }

        } catch (java.util.NoSuchElementException e) {
            throw new EnigmaException(e + " invalid input");
        }
    }
